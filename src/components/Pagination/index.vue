<template>
    <div class="pagination">
        ...
        <button v-if="startNumAndEndNum.end < totalPage "
                @click="$emit('getPageNo',totalPage)" :class="{active:pageNo==totalPage}">{{totalPage}}
        </button>
...
<script>
    export default {
        ...
        computed: {
            ...
            startNumAndEndNum() {
                /*分页器组件需要知道我一共展示多少条数据 ----total
                每一个需要展示几条数据------pageSize
                需要知道当前在第几页-------pageNo
                ...
                * */
                ...
                //        先定义两个变量存储起始数字与结束数字
                let start = 0, end = 0;
                //    连续页码数字5【就是至少5页】，如果出现不正常数字（就是不够5页）
                //不正常现象【总页数没有连续页码多】
