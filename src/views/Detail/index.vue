            ...
            <div class="mainCon">
                ...
                <div class="InfoWrap">
                    ...
                    <div class="choose">
                        ...
                        <div class="cartWrap">
                            <div class="controls">
                                <input autocomplete="off" class="itxt" v-model="skuNum" type="number"
                                       @change="changSkuNum($event)">
                                <a href="javascript:" class="plus" @click="skuNum++<99">+</a>
                                <a href="javascript:" class="mins" @click="skuNum>1?skuNum--:skuNum=1">-</a>
                            </div>
                            <div class="add">
                                <!--以前咋们的路由跳转，从A路由跳转到B路由，这里在加入购物车，进行路由跳转之前
...
<script>
    ...
    export default {
        ...
        methods: {
            //产品的售卖属性值切换高亮
            //arr就是相当于兄弟，js中获取所有得兄弟节点,saleAttrVale是点击得属性值
            changeActive(saleAttrVale, arr) {

                // 遍历全部售卖属性值isChecked为0就没有高亮了
                arr.forEach(item => {
                    console.log(arr,'1',item,'2')
                    item.isChecked = 0;
                });
      ...
      //2:你需要知道这次请求成功还是失败，如果成功进行路由跳转，如果失败，需要给用户提示
                * */
                ...
                try {
                    ...
                    //进行路由跳转
                    //  在路由跳转的时候将
                    // 本地存储 和会话存储   是html5新增的属性
                    //  本地存储有上限内存的 最高是5m   持久化
                    //  会话存储  浏览器一关闭会话就结束
                    //  一些简单的数据skuNum,通过query形式给路由组件传递过去
                    //  产品信息的数据(比较复杂:skuInfo)，通过会话存储（不做持久化，会话结束数据消失）
                    //  本地存储|会话存储，一般存储的是字符串 不允许存对象
