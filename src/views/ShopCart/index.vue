    ...
    export default {
        ...
        methods: {
            ...
            handler: throttle(async function (type, disNum, cart) {
                //    type：为了区分这三个怨怒
                //    disNum新参：+ 变换量(1) -变化量(-1) input最终得个数(并不是最终量)
                //    cart：哪一个产品(身上有id的那个)
                //    向服务器发请求，修改数量
                switch (type) {
                    //加号
                    ...
                    case 'change':
                        ...
                        if (isNaN(disNum) || disNum < 1) {
                            ...
                            //属于正常情况(小数，取整)，带给服务器变化的量 用户输入进来的 - 产品起始的量
...
//                    如果删除成功再次发请求获取新的数据进行展示
            ...
            updateChecked(cart, event) {
                ...
                try {
                  ...
                  let result =  this.$store.dispatch('updateCheckedById', {skuId: cart.skuId, isChecked});
                    this.getData();
                } catch (e) {
                    alert(e.message)
                }
        ...
        computed: {
            ...
            isAlLCheck() {
                //遍历数组里面的元素   只要全部元素的isChecked元素=1返回值就是true  只要有一个为不为1 那返回就为假
